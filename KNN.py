...
#KNN classifier on the Malicious and Benign Websites dataset.
...
# #define parameters to try for KNN
...
# grs = GridSearchCV(KNeighborsClassifier(), param_grid=params, cv = 10, 
#                   n_jobs = -1)
...
##model = KNeighborsClassifier(). This ensured that the initial comparison 
##between models were fair, as NB cannot use gridSearchCV

##define model with HPT (test size = 0.3)
##Best Hyper Parameters: {'algorithm': 'auto', 'metric': 'canberra', 
##'n_neighbors': 3, 'p': 1, 'weights': 'uniform'}
model = KNeighborsClassifier(algorithm = 'auto', metric = 'canberra', 
                             n_neighbors = 3, p = 1, weights = 'uniform')
