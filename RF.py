...
#RF classifier on the Malicious and Benign Websites dataset.
...
# #define best parameters to try for RF
...
# grs = GridSearchCV(RandomForestClassifier(), param_grid=params, cv = 10,
#                   n_jobs = -1)
...
##please note: initial results were discovered with 
##model = RandomForestClassifier(). This ensured that the initial comparison 
##between models were fair, as NB cannot use gridSearchCV

##define model with HPT (test size = 0.3)
##Best Hyper Parameters: {'bootstrap': 'False', 'class_weight': None, 
##'criterion': 'entropy', 'max_features': 'log2', 'max_samples': 0.9, 
##'min_samples_split': 0.1, 'n_estimators': 50, 'oob_score': 'True'}
model = RandomForestClassifier(bootstrap = 'False', class_weight = None, 
                                criterion = 'entropy', max_features = 'log2', 
                                max_samples = 0.9, min_samples_split = 0.1, 
...
m_recall = report['1']['recall'] #this = 0.32
...
#overall score
overall_score = (0.32 + 1 - (cv_variance * 1000) + auc) / 3
